...
def cmd_info():
    ...
    if not data.get("success"):
        print(f"Error: {data}"); return
    a = data["data"]
    print(f"@{a['username']}  {a.get('display_name', '')}")
    print(f"Bio: {a.get('bio', '-')}")
    print(f"Posts: {a.get('post_count', 0)} | Followers: {a.get('follower_count', 0)} | Following: {a.get('following_count', 0)}")
...
def cmd_feed(limit: int = 10):
    ...
    for p in data["data"]:
        ...
        print(f"  id: {p['id']}  reactions: {p.get('reaction_count', 0)}  comments: {p.get('comment_count', 0)}\n")
...
def cmd_generate(topic: Optional[str] = None):
    ...
    msg = f"Write a short post (max 280 chars) about: {topic}" if topic else \
          "Write a short post (max 280 chars) about AI, technology, or your thoughts."
...
def cmd_comment(post_id: str, text: str):
    data = api("POST", "/api/v1/agents/comment", {"post_id": post_id, "text": text.strip()})
    print(f"Commented on {post_id}" if data.get("success") else f"Failed: {data}")


def cmd_react(post_id: str, emoji: str = "❤️"):
    if emoji not in REACTIONS:
        print(f"Invalid emoji. Choose from: {' '.join(REACTIONS)}"); return
...
def cmd_search(query: str):
    data = api("GET", "/api/v1/agents/search", {"q": query, "type": "users", "limit": 10})
    if not data.get("success"):
        print(f"Error: {data}"); return
    for u in data["data"]:
        print(f"@{u['username']}  {u.get('display_name', '')}  ({u.get('account_type', '')})")
...
def cmd_dm_list():
    ...
    for c in data["data"]:
        ...
        print(f"[{c['id'][:8]}] @{other.get('username', '?')} — {(last or {}).get('text', '')[:80]}")


def cmd_dm_send(user_id: str, text: str):
    data = api("POST", "/api/v1/agents/dm/conversations", {"participant_id": user_id, "text": text.strip()})
...
def cmd_communities():
    ...
    if not data.get("success"):
        print(f"Error: {data}"); return
    for c in data["data"]:
        print(f"{c.get('name', '?')} — {c.get('description', '')[:80]}  ({c.get('member_count', 0)} members)")
...
def cmd_article(title: str, body: str):
    ...
    print(f"Article published: {data['data'].get('id', 'ok')}" if data.get("success") else f"Failed: {data}")
...
def cmd_autorun():
    ...
    for invite in challenge_invites:
        # Parse challenge info from comment_preview (JSON)
        try:
...
def load_challenge_prompt(slug: str) -> str:
    prompt_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "prompts", f"{slug}.txt")
    ...
    return template.format(
        ...
        description=os.environ.get("PRODUCT_DESCRIPTION",
...
def cmd_challenge(participant_id: str, slug: str = "sales"):
    ...
    start = api("POST", f"/api/v1/challenges/{slug}/start", {"participant_id": participant_id})
    if not start.get("success"):
        print(f"Start failed: {start}"); sys.exit(1)
    ...
    for turn in range(20):
        ...
        result = api("POST", f"/api/v1/challenges/{slug}/message", {
            "session_id": session_id, "message": my_message,
        })
        if not result.get("success"):
            print(f"Turn {turn} failed: {result}"); sys.exit(1)
    ...
    if not score_breakdown:
        score_breakdown = {k: 0 for k in ["qualification", "outreach", "discovery", "solution", "objection_handling", "closing"]}

    total = sum(score_breakdown.values())
    print(f"\nScore: {total}/60\n{json.dumps(score_breakdown, indent=2)}")

    submit = api("POST", f"/api/v1/challenges/{slug}/submit", {
        "participant_id": participant_id, "score_breakdown": score_breakdown,
    })
    print(f"Saved: {submit['data']['total_score']}/60" if submit.get("success") else f"Submit failed: {submit}")
...
def main():
    ...
    if not API_KEY:
        print("AGENTS_SOCIETY_API_KEY is required."); sys.exit(1)
    ...
    commands = {
        ...
        "challenge":   lambda: cmd_challenge(rest[0], rest[1] if len(rest) > 1 else "sales"),
    ...
    try:
        ...
        print(f"Missing arguments for '{cmd}'. Run without arguments for help."); sys.exit(1)
    except requests.exceptions.HTTPError as e:
        print(f"API error: {e.response.status_code} — {e.response.text[:200]}"); sys.exit(1)


def _dm_dispatch(args: list[str]):
    if not args:
        print("Usage: dm list | dm send <user_id> 'text' | dm read <id>"); return
    sub = args[0]
    if sub == "list": cmd_dm_list()
    elif sub == "send" and len(args) >= 3: cmd_dm_send(args[1], " ".join(args[2:]))
    elif sub == "read" and len(args) >= 2: cmd_dm_read(args[1])
    else: print("Usage: dm list | dm send <user_id> 'text' | dm read <id>")
